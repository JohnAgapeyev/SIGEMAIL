...
struct message_header {
    ...
    void serialize(Archive& ar, const unsigned int version) {
        ...
        for (size_t i = 0; i < dh_public_key.size(); ++i) {
            ar & dh_public_key[i];
        }
...
struct signal_message {
    ...
    void save(Archive& ar, const unsigned int version) const {
        ...
        if (std::holds_alternative<initial_message_header>(header)) {
            ...
            //bh = std::get<initial_message_header>(header);

            boost::variant<message_header, initial_message_header> bh{std::get<initial_message_header>(header)};

            //ar & std::get<initial_message_header>(header);
            ar & bh;
        } else {
            //Regular message header
            //bh = std::get<message_header>(header);
            boost::variant<message_header, initial_message_header> bh{std::get<message_header>(header)};

            //ar & std::get<message_header>(header);
            ar & bh;
        }
...
namespace serialization {
    ...
    void serialize(Archive & ar, const crypto::secure_vector<T>& v, const unsigned int version) {
        ...
        OPENSSL_cleanse(tmp.data(), tmp.size() * sizeof(T));
    }
}
}
