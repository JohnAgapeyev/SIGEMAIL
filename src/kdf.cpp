...
const std::array<std::byte, 32> crypto::root_derive(std::array<std::byte, 32>& root_key, const std::array<std::byte, 32>& dh_output) {
...
const std::array<std::byte, 32> crypto::chain_derive(std::array<std::byte, 32>& chain_key) {
...
const std::array<std::byte, 32> crypto::x3dh_derive(const std::vector<std::byte>& key_material) {
    //Pad with 32 bytes of 0xFF
    std::vector<std::byte> kdf_input{32, std::byte{0xFF}};

    //Add the key material
    kdf_input.insert(kdf_input.end(), key_material.begin(), key_material.end());

    //Fill with a sha512 worth of zeroes
    std::array<std::byte, 64> kdf_salt{std::byte{0}};

    std::array<std::byte, 32> kdf_output;
